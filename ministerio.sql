...
alter table ACTIVIDAD add constraint FK_DEDICA foreign key (CI)
...
alter table CLASIFICACION add constraint FK_TIENE foreign key (ID_PROPIEDAD)
      references PROPIEDAD (ID_PROPIEDAD) on delete restrict on update restrict;

alter table ESTADO add constraint FK_ENCUENTRA foreign key (ID_TRAMITE)
...
alter table PERSONAL add constraint FK_ASIGNA foreign key (ID_TRAMITE)
      references TRAMITE (ID_TRAMITE) on delete restrict on update restrict;
